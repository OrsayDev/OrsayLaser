...
class SirahCredoLaser:
    ...
    def __init__(self, SERIAL_PORT='COM12') -> None:
        ...
        try:
            ...
            print("***LASER***: Could not open serial port. Entering in debug mode...")
    ...
    def _set_hardware_wl(self, wl):
        if not self.debug:
            pos = self._wl_to_pos(wl)
    ...
    def setWL(self, wavelength: float, current_wavelength: float):
        if (abs(float(current_wavelength) - float(wavelength)) <= 0.001):
            ...
            self.laser_thread = threading.Thread(target=self._set_hardware_wl, args=(wavelength,))
    ...
    def set_scan_thread(self, cur, i_pts, step):
        if not self.abort_ctrl:
            self.lock.acquire()
            self._set_hardware_wl(cur + i_pts * step)
        else:
            with self.lock:
